...
private val ZERO = 0.toExDouble()

fun main() {

    data class P(val x: ExDouble, val y: ExDouble, val z: ExDouble) {
        operator fun plus(other: P) = P(x + other.x, y + other.y, z + other.z)
        operator fun minus(other: P) = P(x - other.x, y - other.y, z - other.z)
        operator fun div(other: ExDouble) = P(x / other, y / other, z / other)
        operator fun times(other: ExDouble) = P(x * other, y * other, z * other)
    }
    ...
    fun calcIntersectionXyz(stone1: Hailstone, stone2: Hailstone): P? {
        ...
        return when {
            intersectionFrom1 != intersectionFrom2 -> {
                "intersectionFrom1: $intersectionFrom1".println()
                "intersectionFrom2: $intersectionFrom2".println()
                null
            }
    ...
    fun parseHailstones(input: List<String>): List<Hailstone> {
        return input.map { line ->
            val ns = "[-\\d]+".toRegex().findAll(line).map { it.value.toExDouble() }.toList()
    ...
    fun part1(input: List<String>, range: LongRange): Int {
        ...
        for (index in 0..<hailstones.lastIndex) {
            ...
            for (h2 in hailstones.subList(index + 1, hailstones.size)) {
                ...
                if ((intersection.x < range.first.toExDouble())
                    || (intersection.x > range.last.toExDouble())
                    || (intersection.y < range.first.toExDouble())
                    || (intersection.y > range.last.toExDouble())
                ) {
                    continue
                }
